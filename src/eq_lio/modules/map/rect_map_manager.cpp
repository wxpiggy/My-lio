...
namespace EQLIO {
    RectMapManager::RectMapManager(const std::string &config_file_path, const std::string &prefix)
        ...
        kdtree_ptr = std::make_shared<KD_TREE<EQLIO::Point>>();
...
void RectMapManager::addScan(PCLPointCloudPtr curr_scan, const Eigen::Quaterniond &att_q,
    ...
    if (local_map_ptr->empty()) {
        *local_map_ptr = scan;
        ...
        std::vector<EQLIO::Point, Eigen::aligned_allocator<EQLIO::Point>> new_points;  // 本次新增点缓存
        for (auto &&point : scan) {
            std::vector<EQLIO::Point, Eigen::aligned_allocator<EQLIO::Point>> ind;
