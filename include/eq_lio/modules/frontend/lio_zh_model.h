...
#include "eq_lio/math/geometry.h"
#include <cmath> // isnan, sqrt
using namespace liepp;

namespace EQLIO{
    class LIOZHModel : public EQKF::CalcZHInterface{
    private:
    ...
    public:
        ...
        bool calculate(const EQKF::State18 &state, Eigen::MatrixXd &Z, Eigen::MatrixXd &H) override{
    ...
    #pragma omp parallel for
            ...
            for (size_t i = 0; i < current_cloud_ptr->size(); i++)
            {
                ...
                std::vector<Point,Eigen::aligned_allocator<EQLIO::Point>> nearest_pts;
